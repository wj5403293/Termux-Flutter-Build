...
GitHub Actions 現在只保留手動觸發。

發布版本號完全來自倉庫中的 `build.toml`：
...
修改這個值之後手動執行 workflow，流程會自動：

- clone 對應 Flutter tag
- 套用 `patches/` 根目錄下固定的 4 個 patch
- 構建 `.deb`
- 發布同名 GitHub Release tag：`3.41.5`
...
Linux desktop 與 `--profile` 不屬於預設 `termux` 包。如果你需要這些能力，仍可在手動 workflow 中選擇以下 preset：
